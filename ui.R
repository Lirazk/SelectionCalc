...
plot_panel <- div(class = "well",
  fluidRow(column(4,
    ...
    #   value = 0.3
    # )),
    column(4, slider_and_numeric("r", "$$R^2:$$", 0, 1, NULL, 0.3, 
    ...
    conditionalPanel(
      ...
      #   value = 0.3
      # ))
      slider_and_numeric("q", "Percentile from which to exclude embryos:", 0.01, 1, 0.01, 0.3, paste("Embryos with PRS above that percentile are excluded. For example, if the parameter equals 10%, all embryos with PRS at the top 10% of the distribution of PRS in the population will be excluded. If no embryo is below the threshold we select one randomly."))
    )),
  ),
...
calc_panel <- div(class = "well",
  ...
  fluidRow(column(4, slider_and_numeric("N2", "Number of embryos:", 2, 10, 1, 5, "The number of embryos available for selection."),
                  ...
                  slider_and_numeric("r2", "$$R^2:$$", 0, 1, NULL, 0.3, "The proportion of the variance in the liability of the disease explained by the polygenic risk score. It is a measure of the accuracy of the score. Typically in the range 0.05-0.1.")),
           ...
           conditionalPanel(
             ...
             slider_and_numeric("q2", "Percentile from which to exclude embryos:", 0.01, 1, 0.01, 0.3, paste("Embryos with PRS above that percentile are excluded. For example, if the parameter equals 10%, all embryos with PRS at the top 10% of the distribution of PRS in the population will be excluded. If no embryo is below the threshold we select one randomly.")),
           ), fluidRow(column(10, offset = 2, htmlOutput("summary"), align = "center"))),
           column(4,
           conditionalPanel(
             condition = "input.type2 == 'Conditional'",
             slider_and_numeric("qf2", "Father's polygenic risk score percentile:", 0, 1, 0.01, 0.5, paste("For example, if this parameter equal 0.05, the PRS of the father is at the top 5% of the distribution of PRS in the population.")),
             slider_and_numeric("qm2", "Mother's polygenic risk score percentile:", 0, 1, 0.01, 0.5, paste("For example, if this parameter equal 0.05, the PRS of the mother is at the top 5% of the distribution of PRS in the population.")),
           ),
           conditionalPanel(
             condition = "input.type2 == 'Family History'",
             slider_and_numeric("h2", "$h^2:$", 0, 1, 0.01, 0.4, "The heritability of the disease. Only relevant when conditioning on the parents' diseases status"),
             checkboxInput("df2",
...
calc_two_traits <- div(class = "well", fluidRow(column(4,
                                                ...
                                                column(4, 
                                                       slider_and_numeric("r2_1", "$$R^2 ~ \\text{disease 1:}$$", 0, 1, 0.001, 0.3, "The proportion of the variance in the liability of the first disease explained by the polygenic risk score. It is a measure of the accuracy of the score. Typically in the range 0.05-0.1."),
                                                       slider_and_numeric("r2_2", "$$R^2 ~ \\text{disease 2:}$$", 0, 1, 0.001, 0.3, "The proportion of the variance in the liability of the second disease explained by the polygenic risk score. It is a measure of the accuracy of the score. Typically in the range 0.05-0.1."),
